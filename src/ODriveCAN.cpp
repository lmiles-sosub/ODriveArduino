...
bool ODriveCAN::setPositionAbs(float position) {
    Set_Absolute_Position_msg_t input_pos_msg;
    input_pos_msg.Position = position;
    return send(input_pos_msg);
}
...
void ODriveCAN::onReceive(uint32_t id, uint8_t length, const uint8_t* data) {
    ...
    while (byte_index >= 0) Serial.print(data[byte_index--], HEX);
    Serial.println("");
#endif // DEBUG
    ...
    switch(id & ODriveCAN::kCmdIdBits) {
        ...
        case  Get_Iq_msg_t::cmd_id: {
            ...
            Get_Iq_msg_t status;
            status.decode_buf(data);
            if (iq_callback_ != nullptr) iq_callback_(status, iq_user_data_);
            else Serial.println("missing callback");
            break;
        }
       
        
        case TxSdo_msg_t::cmd_id: {
            TxSdo_msg_t status;
            status.decode_buf(data);
            if (TxSdo_callback_ != nullptr) TxSdo_callback_(status, TxSdo_user_data_);
            else Serial.println("missing callback");
            //if the 
            if ((id & ODriveCAN::kCmdIdBits) != requested_msg_id_) break;
            switch(status.Endpoint_ID){
                case SPI_ENCODER0_RAW:{
                    if (RawEncoder_callback_ != nullptr) RawEncoder_callback_(status, RawEncoder_user_data_);
                    break;
                }
                /*
                case POS_ESTIMATE:{
            ...
            requested_endpoint_id_ = REQUEST_PENDING_ENDPOINT_ID;
            break;
        }
        
        case RxSdo_msg_t::cmd_id: {
            RxSdo_msg_t status;
            break;
        }
        default: {
...
bool ODriveCAN::awaitMsg(uint16_t timeout) {
    ...
    while (requested_msg_id_ != REQUEST_PENDING && requested_endpoint_id_ != REQUEST_PENDING_ENDPOINT_ID) {
