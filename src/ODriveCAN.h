...
#define REQUEST_PENDING_ENDPOINT_ID 0xff
...
public:
    ...
    bool setPosition(float position, float velocity_feedforward = 0.0f, float torque_feedforward = 0.0f);
        ...
        /**
     * @brief Sends a position value to update the odrives position.
     * 
     * This function returns immediately and does not check if the ODrive
     * received the CAN message.
     */
    bool setPositionAbs(float position);
    ...
    void onStatus(void (*callback)(Heartbeat_msg_t& feedback, void* user_data), void* user_data = nullptr) {
     ...
     /**
     * @brief Registers a callback for ODrive TxSdo
     */
    void onTxSdo(void (*callback)(TxSdo_msg_t& feedback, void* user_data), void* user_data = nullptr) {
        TxSdo_callback_ = callback; 
        TxSdo_user_data_ = user_data;
    }

         /**
     * @brief Registers a callback for ODrive raw Encoder
     */
    void onRawEncoder(void (*callback)(TxSdo_msg_t& feedback, void* user_data), void* user_data = nullptr) {
        RawEncoder_callback_ = callback; 
        RawEncoder_user_data_ = user_data;
    }

    /**
     * @brief Registers a callback for ODrive raw Encoder
     */
    void onIQ(void (*callback)(Get_Iq_msg_t& feedback, void* user_data), void* user_data = nullptr) {
        iq_callback_ = callback; 
        iq_user_data_ = user_data;
    }
    ...
    /**
     * @brief awaits for a certain message.
     * 
     * Blocks until the response is received or the timeout is reached. Returns
     * false if the ODrive does not respond within the specified timeout.
     */
    template<typename T>
    bool requestTxdo(T& msg, uint16_t timeout_ms = 10) {
        requested_msg_id_ = msg.cmd_id;
        requested_endpoint_id_ = msg.Endpoint_ID;
        if (!awaitMsg(timeout_ms)) return false;
        msg.decode_buf(buffer_);
        return true;
    }
...
private:
    ...
    volatile uint8_t requested_endpoint_id_ = REQUEST_PENDING_ENDPOINT_ID;
    ...
    void* TxSdo_user_data_;
    void* RawEncoder_user_data_;
    void* iq_user_data_;
    ...
    void (*TxSdo_callback_)(TxSdo_msg_t& feedback, void* user_data) = nullptr;
    void (*RawEncoder_callback_)(TxSdo_msg_t& feedback, void* user_data) = nullptr;
    void (*iq_callback_)(Get_Iq_msg_t& feedback, void* user_data) = nullptr;
};
